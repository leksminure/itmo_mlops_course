...
def evaluate_models(model_test_pairs, output_dir="results"):
...
if __name__ == "__main__":
    ...
    for model_dir in models_path.iterdir():
        if model_dir.is_file():
            ...
            test_data = pd.read_csv(os.path.join(args.data_path, dataset_name, "test_df.csv")).dropna()
            model_test_pairs.append((os.path.join(models_path, model_file_path), test_data))

    output_dir = Path(args.output_path)
    output_dir.mkdir(parents=True, exist_ok=True)

    results_df = evaluate_models(model_test_pairs)
    results_df.to_csv(os.path.join(args.output_path, "summary.csv"), index=False)
